...
# Bölge → iller eşleşmesi (poligonu olmayan eski bölge müdürleri için)
BOLGE_ILLER = {
...
def bolge_dashboard(db: Session = Depends(get_db), current_user: models.User = Depends(bolge_auth)):
    ...
    # Kendi bölge poligonunu al
    from routers.bolge_yonetim import BolgeModel as BM3
    kendi_bolge_dash = db.query(BM3).filter(BM3.mudur_id == current_user.id, BM3.aktif == 1).first()
    
    kesisen_acil = []
    for ab in acil_bolgeler:
        kesisiyor = False
        
        if kendi_bolge_dash and kendi_bolge_dash.koordinatlar:
            # Geometrik kesişim — acil bölge köşeleri kendi poligonda mı?
            for nokta in ab.koordinatlar:
                if pip(nokta[0], nokta[1], kendi_bolge_dash.koordinatlar):
                    kesisiyor = True
                    break
            # Kendi poligon köşeleri acil bölgede mi?
            if not kesisiyor:
                for nokta in kendi_bolge_dash.koordinatlar:
                    if pip(nokta[0], nokta[1], ab.koordinatlar):
                        kesisiyor = True
                        break
        else:
            # Poligonu olmayan eski bölge müdürleri için il merkez noktalarını kullan
            bolge_adi_metin = current_user.bolge or ""
            # Büyük/küçük harf uyumsuzluğunu gider
            iller = next((v for k, v in BOLGE_ILLER.items() if k.lower() == bolge_adi_metin.lower()), [])
            for il in iller:
                koord = IL_MERKEZ_KOORD.get(il)
                if koord and pip(koord[0], koord[1], ab.koordinatlar):
                    kesisiyor = True
                    break

        if kesisiyor:
            acil_merkezler2 = [m for m in (db.query(models.Merkez).filter(models.Merkez.aktif == True).all())
            ...
            kesisen_acil.append({
                ...
                "merkez_sayisi": len(acil_merkezler2),
            })
...
def bolge_harita(db: Session = Depends(get_db), current_user: models.User = Depends(bolge_auth)):
    ...
    # Kullanıcının bölge poligonunu al
    from routers.bolge_yonetim import BolgeModel as BM2
    kendi_bolge = db.query(BM2).filter(BM2.mudur_id == current_user.id, BM2.aktif == 1).first()
    
    for ab in acil_bolgeler_raw:
        kesisiyor = False
        
        # Operasyon müdürü kendi bölgesini her zaman görür
        if current_user.role == models.UserRole.OPERASYON_MUDUR:
            kesisiyor = True
        # Kendi bölge poligonu varsa geometrik kesişim kontrolü
        elif kendi_bolge and kendi_bolge.koordinatlar:
            # Acil bölgenin herhangi bir köşesi kendi bölgede mi?
            for nokta in ab.koordinatlar:
                if pip(nokta[0], nokta[1], kendi_bolge.koordinatlar):
                    kesisiyor = True
                    break
            # Kendi bölgenin herhangi bir köşesi acil bölgede mi?
            if not kesisiyor:
                for nokta in kendi_bolge.koordinatlar:
                    if pip(nokta[0], nokta[1], ab.koordinatlar):
                        kesisiyor = True
                        break
        else:
