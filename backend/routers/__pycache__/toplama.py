...
def tir_ulasti(
    ...
    araclar = db.query(models.Arac).filter(models.Arac.plaka == tir.plaka).first()
    if araclar:
        araclar.durum = models.AracDurum.MUSAIT
    # Şoförü bul (sofor_ad ile eşleştir)
    if tir.sofor_ad:
        ad_parcalar = tir.sofor_ad.strip().split()
        if len(ad_parcalar) >= 2:
            sofor = db.query(models.Sofor).filter(
                models.Sofor.ad == ad_parcalar[0],
                models.Sofor.soyad == ad_parcalar[-1]
            ).first()
            if sofor:
